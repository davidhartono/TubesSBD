        ...
        <div class="container">
            <div class="row mt-5">
                <div class="col-10 text-end">
                    @if (Auth::check() && $author_id === Auth::user()->id)
                        <button class="btn btn-secondary me-2" onclick="window.location='{{ url('tips/' . $id . '/edit') }}'">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <form class="d-inline" action="{{ url('tips/' . $id) }}" method="post"
            ...
            <div class="row mt-3 justify-content-center">
                <div class="col items-center text-center">
                    @if ($image)
                        <img src="{{ asset('img/tip/' . $image) }}" alt="{{ $title }}" width="660"
                            class="rounded">
                    @endif
                </div>
            </div>
                ...
                <div class="col-8 items-center text-left">
                    <div class="card">
                        <div class="card-body">
                            ...
                            @endforeach
                            @auth
                                <form action="{{ route('comments.tips') }}" class="form-inline" method="POST">
                                    @csrf
                                    @include('komponen.pesan')
                                    <div class="input-group w-100">
                                        <input type="hidden" name="tips_id" value="{{ $id }}">
                                        <input type="text" name="komentartips" class="form-control rounded border-right-0"
                            ...
                            @else
                                <p class="text-muted">Please <a href="{{ url('/login') }}" class="text-warning">login</a> to
                                    leave a comment.</p>
                            @endauth
                        </div>
                    </div>
